...
type searchBarType = { searchName?: string|undefined, dropdowns?: dropdown[]|undefined }

export default function SearchBar({searchName, dropdowns}: searchBarType) {
    return (
        <div className="md:flex gap-5 items-center mb-5 justify-end">
            
            {(searchName!==undefined) && <SearchInput name={searchName} />}
                ...
                dropdowns.map(d => {
                    return (
                        <div className="relative block">
                            <label className="">
                                {d.label}
                                <select name={d.name} className="block w-40 h-10 rounded-lg border p-2 text-sm focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-400">
...
export function SearchInput({name}: {name: string}){
    return (
        <div className="relative h-10">
            <input name={name} type="text" placeholder="Search..." className="block w-64 rounded-lg border py-2 pl-10 pr-4 text-sm focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-400" />
